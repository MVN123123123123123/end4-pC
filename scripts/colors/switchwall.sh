...
get_video_opts() {
    ...
    if [ -f "$SHELL_CONFIG_FILE" ]; then
        ...
        local config_mute=$(jq -r '.background.video.mute // true' "$SHELL_CONFIG_FILE" 2>/dev/null)
        [[ -n "$config_mute" && "$config_mute" != "null" ]] && mute="$config_mute"

        local config_loop=$(jq -r '.background.video.loop // true' "$SHELL_CONFIG_FILE" 2>/dev/null)
